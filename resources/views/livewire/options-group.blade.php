...
<div>
    ...
    <x-baseview title="{{ __('Options Group') }}">
    ...
    <div x-data="{ open: @entangle('showEdit') }">
        <x-modal confirmText="{{ __('Update') }}" action="update">
            ...
            <x-input title="{{ __('Name') }}" name="name" />
            <x-checkbox title="Multiple" description="{{ __('Allow customers to select multiple options under this option group') }}" name="multiple" :defer="false" />
            <x-checkbox title="{{ __('Active') }}" name="isActive" :defer="false" />
