@section('title', __('Products') )
    <div>
        ...
        <x-baseview title="{{ __('Products') }}">
            <livewire:tables.product-table />
        </x-baseview>

        {{-- new form --}}
        <div x-data="{ open: @entangle('showCreate') }">
            <x-modal-lg confirmText="{{ __('Save') }}" action="save">
                ...
                <x-media-upload title="{{ __('Image') }}" name="photo" :photo="$photo" :photoInfo="$photoInfo" types="PNG or JPEG"
                    rules="image/*" />
                ...
                @endphp
                @endrole
                <x-select title="{{ __('Vendor') }}" :options="$vendors" name="vendorID" />
                ...
                <div class="grid items-center grid-cols-2 space-x-4">
                    @role('admin')
                    <x-checkbox title="{{ __('Featured') }}" name="featured" description="{{ __('Featured on app home screen') }}" :defer="false" />
                    @endrole
        ...
        <div x-data="{ open: @entangle('showEdit') }">
            <x-modal-lg confirmText="{{ __('Update') }}" action="update">
                ...
                <x-media-upload title="{{ __('Image') }}" name="photo" preview="{{ $selectedModel->photo ?? '' }}" :photo="$photo"
